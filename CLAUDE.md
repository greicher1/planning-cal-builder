...
There is a runner now, and fixtures. `tests/harness/` holds a headless-Chrome harness: `run.sh` drives one test, `gate.sh` runs the whole acceptance gate and diffs it against `tests/baselines/2026-08-29-stage-7/` and, for the month PDF, `tests/baselines/2026-09-22-monthprint/`. Since 22 Sep 2026 it takes ~3 minutes, not ~50: `run.sh` polls for Chrome's dump rather than waiting out a wall-clock timer. `tests/fixtures/` holds real saved calendars for the restore path. See PROJECT-CONTEXT §11 and UI-CONVENTIONS §10.
...
⚠️ **`tests/fixtures/` now holds `.sptcal` fixtures (`colswap-*`, `stintswap-*`), but every one of them is loaded through the INLINE saved-state path (`?state=`), not the picker.** That path converges on the same `applyStateSnapshot` the picker uses, so the snapshot schema is exercised; the picker itself is proven only by the legacy `restore` leg on `v1.0.0-saved.html`. ✅ **That leg RUNS again as of 8 Sep 2026** — it had failed on every run because `appReady()` waited for `renderRecents()`, which is behind an `indexedDB.open` that never settles in headless Chrome. The stall is real and unfixed, but it never blocked the Open path itself: `#file-menu` is `keepMounted`, so the `Open…` item is clickable while the wrap is hidden, and the delegated listener is on `#file-menu`. The probe now waits on engine-generated sidebar markup instead. ⛔ **A `.sptcal` through the real picker is STILL the missing insurance for §0 rule 3** — `restore` opens the legacy `.html`. It is now a short leg rather than a project, because the picker path is provably drivable.
...
**Run and prove the BUILD:** `npm run dev` serves `src/` for iteration; `npm run build` produces `dist/index.html` (one self-contained file, ~983 KB). Before calling any change to the grid, the exports or the save path done, run `cd tests/harness && ./gate.sh` — it is the gate the freeze rule above demands, and it compares the waterfall PDF, every Excel part and the month PDF (the printed document, plus one printed sheet per month) against the baselines.
...
| `episodeShootOrder` | the episodes' SHOOTING order, as ids (ruling 9). `[]` = natural order; read only through `effectiveShootOrder()`. A separate store because episode names derive from their POSITION in `episodeDefs` — never reorder that array. Labels only: it never changes Production's length |

**Not** state, deliberately: `autosaveNeedsFile`, `autosaveFailed`, `isDirty`, `suppressDirty` and the legacy-notice visibility are all session UI, and none belongs in a saved file. Per-user *preferences* (`SHEET_GRIDLINES`, `WF_PDF_MODE`, `GRID_TEXT_COLOR`, and since 22 Sep 2026 `prefs.mvBlocks` / `prefs.mvEpisodes` — the month view's grey pill text) belong in `localStorage`, **never** in `captureSnapshot()` — they are not calendar data and must not travel inside someone else's file. ⛔ `SHEET_GRIDLINES` is the first one built (3 Sep 2026, key `sptcal.prefs`): its control lives in the **Preferences** card, whose `.prefs-card` class is what `collectFieldValues()` skips. **A preference control outside that card is silently baked into every saved calendar.**
