...
- **Region model:** a country (`#union-country`: `US` / `CA` / `UK`) plus, for Canada only, a province (`#union-subregion`: `CA-BC`, `CA-ON`, `CA-QC`, `CA-AB`, `CA-MB`, `CA-NS`). `effectiveRegionKey()` resolves the pair to one `HOLIDAYS` key; `reflectRegionUI()` shows/hides the province row; `normalizeRegionSelection()` rewrites the legacy `CAN` value from pre-split saves onto `CA` + a province; `syncRegionTracking()` re-baselines the change-guard after any programmatic load/restore.
- `HOLIDAYS` (~line 999) is keyed by **region**, not country, and is **generated from holiday rules** rather than hand-transcribed — regenerate it rather than editing dates by hand. US is the IATSE Basic Agreement's 11 recognized holidays; **Columbus Day and Veterans Day are intentionally absent** (Veterans Day is AICP-commercial-only; Columbus Day is on no IATSE calendar). Canada is per-province because the statutory lists genuinely differ (Boxing Day is ON-only; Remembrance Day BC/AB; Truth & Reconciliation BC/MB; Fête nationale QC-only). Weekend holidays also emit an `(Observed)` weekday entry — US shifts Sat→Fri / Sun→Mon per the IATSE ASA rule, Canada/UK move forward to the next free weekday.
