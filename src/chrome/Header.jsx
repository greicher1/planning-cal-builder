...
export function Header() {
  ...
  return (
    <Group className="app-toolbar" gap="sm" wrap="nowrap" align="center" w="100%">
      ...
      <Menu
          opened={menu.open}
          onChange={(open) => setMenu((s) => ({ ...s, open }))}
          withinPortal={false}
          keepMounted
        ...
        >
          <Box id="file-menu-wrap" className="file-menu-wrap"
            ...
            <Menu.Dropdown id="file-menu" role="menu" className="file-menu">
              {menu.items.length === 0 ? (
                ...
                menu.items.map((f) => (
                  <Menu.Item
                    ...
                    role="menuitem"
                    title={f.name}
                    /* Native title=, deliberately kept: it reveals a file name the 220px button and
                       the 280px dropdown both truncate. UI-CONVENTIONS §6 keeps native title
                       wherever it surfaces text the layout cut off. */
                    rightSection={
                      /* Not a Menu.Item child by accident -- as rightSection it is a real <button>
                         OUTSIDE the item's label, so the engine's `.closest('[data-remove]')`
                         branch fires before its `.closest('.file-menu-item')` branch, which is the
                         order that keeps the menu open while pruning several entries. */
                      <CloseButton
                        component="span"
                        size="xs"
                        className="fm-remove"
              ...
              >
                <span className="fm-name">Export shareable copy…</span>
              </Menu.Item>
            </Menu.Dropdown>
          </Box>
      </Menu>

      <Button {...BTN} id="new-file-btn"  variant="default">New</Button>
      ...
      {/* ml="auto" replaces `.tb-btn.danger{margin-left:auto}` -- today that single declaration is
          the toolbar's ONLY right-alignment mechanism, so it has to be reproduced explicitly. */}
      <Button {...BTN} id="reset-btn"  variant="default" color="danger" ml="auto">
        Reset All
      </Button>
    </Group>
  )
