...
strips **five** popover classes from its clone — `.note-pop, .mv-note-pop, .phase-color-pop,
.date-pop, .select-pop` (the last two added with the pop-out date picker and the phase picker) — and
separately **re-hides** `#legacy-notice` / `#update-notice`. Keep those class names, and add to the
list whenever a body-level panel is added, or a stray popover exports into a shareable copy.
...
resolves. ✅ **This hazard is CLOSED in the build (round 7): Inter is embedded**, like Carlito —
`src/styles/inter.css`, one variable woff2 (latin), regenerated by `tools/fetch-inter.py`, with the
Google Fonts link and both preconnects gone. The month PDF's text metrics no longer depend on network
availability, and the measured widths at every weight came out byte-identical to the fonts Google was
serving, so nothing about the metrics changed — only their reliability.

⚠️ **Still true of the DEPLOYED `index.html`**, which fetches Inter (and IBM Plex Mono) from
`fonts.googleapis.com`: there, offline or from `file://`, the measurement falls to `sans-serif` while
the render falls to Segoe UI, and the month PDF's metrics depend on the network. The waterfall's
never did.
...
What *can* lose are ties broken by source order. ⚠️ **As built the order is the other way round, and
deliberately so:** `main.jsx` imports Mantine's **layered** files first and the app's **unlayered**
`legacy.css` last, so every app rule outranks every Mantine rule for normal declarations regardless
of specificity — the ties below cannot arise. They are recorded because the precedence **reverses**
for `!important`, which is the one case still worth checking on a Mantine upgrade. The original
analysis, written when Mantine's styles were expected to be injected **after** the app's static
`<style>`: (1) `body{}` at (0,0,1) — font-family, font-size, line-height,
...
- **Cut a new real fixture** from the last pre-Mantine build, alongside the v1.0.0 one.
  ...
  shape.
- ✅ **`HANDOFF` §2h is FIXED (round 7)** — exactly as this bullet advised, and before any
  persistence work. `buildSavedHtml()`'s clone now re-hides `#legacy-notice` / `#update-notice`
  (`el.hidden = false` removes the attribute, and `outerHTML` serialises attributes), which restores
  v1.0.0's output rather than changing it. The strips themselves stayed static markup. ~~Tangled into a persistence
