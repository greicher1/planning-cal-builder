...
| Item | Status |
|---|---|
| **Grid direct manipulation** | ✅ **FEATURE 1 (batch expand) BUILT 1 Sep 2026** — see [`GRID-DIRECT-MANIPULATION-PLAN.md`](GRID-DIRECT-MANIPULATION-PLAN.md) §5 and the README entry. Marquee/Shift/⌘ selection + `Expand` toolbar button + double-click apply, one undo per batch, no frozen edit, **no save-format surface at all**. ⛔ **FEATURE 2 (column swap) STILL BLOCKED on owner decision D1** — a per-week column changes which label sits in which grid column, i.e. inside `#table-wrap`, which the 29 Aug appearance convention freezes; its only escape is *"unless given specific instructions"*, so the ruling must be explicit and recorded here before any code. D2 is decided (collateral capped at magnitude 1; G3 width stays a hard reject). **Do not start F2 from the plan alone.** |
