...
| Item | Status |
|---|---|
| **Grid direct manipulation** | ✅ **FEATURE 1 (batch expand) BUILT + SHIPPED LIVE 1 Sep 2026** — plan §5. Marquee/Shift/⌘ selection + `Expand` toolbar button + double-click apply, one undo per batch, no frozen edit, **no save-format surface**. Hardened by two adversarial review rounds (13 fixes; see the README entries). 🔨 **FEATURE 2 (column swap): F2-a and F2-b DONE, F2-c/F2-d NEXT.** ✅ F2-a — `gridColSwaps` store, mutual-pointer model, reconciler in `computeSchedule`, 9 save sites, `prove-col-permutation.mjs` (theorem fuzzed over the verbatim `computeBlockLayout` source), `colswap` harness leg. ✅ F2-b — `layoutFingerprint`, the six-check gate, per-pair rejection, `#colswap-notice`, `colswapgate` harness leg. ⛔ **There is still NO GESTURE — nothing in the UI can create a swap**, so the whole feature is inert for users. Next: **F2-c** (`computeSwapRun`, WHOLE-run only per D7, + `canSwap`/`commitSwap`) then **F2-d** (the knob and indicators). Do not skip to F2-d. |
