...
| Item | Status |
|---|---|
| **Grid direct manipulation** | ✅ **FEATURE 1 (batch expand) BUILT 1 Sep 2026** — see [`GRID-DIRECT-MANIPULATION-PLAN.md`](GRID-DIRECT-MANIPULATION-PLAN.md) §5 and the README entry. Marquee/Shift/⌘ selection + `Expand` toolbar button + double-click apply, one undo per batch, no frozen edit, **no save-format surface at all**. 🔨 **FEATURE 2 (column swap) UNBLOCKED 1 Sep 2026 — see the sign-off row below.** Build it as plan §6, Phase 1 only, in the increments plan §8 lists (F2-a store+reconciler → F2-b gate → F2-c whole-run swap → F2-d gesture), **not as one commit**. |
